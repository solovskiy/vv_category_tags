...
### Self-heal события
Модуль сам следит за регистрацией события. При срабатывании он проверяет
триггер в `oc_event` и, если тот не равен `catalog/view/*/before` (например,
остался старый узкий триггер после установки прежней версии), чинит его одним
`UPDATE`. Результат кешируется (`vv_category_tags.event_ok`, TTL сутки), так что
проверка выполняется фактически один раз. Благодаря этому достаточно **залить
обновлённый файл контроллера** — переустановка модуля и ручной SQL не нужны.
Исправленный триггер начинает действовать со следующего запроса.
